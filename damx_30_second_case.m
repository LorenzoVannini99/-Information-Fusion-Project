...
%Create the Planned Path
%Create a path to drive through the map for gathering range sensor readings.
path_2= [ 16 20; 22 22 ; 24 16; 16 15];
...
%Follow Path and Map Environment
%Set the initial pose and final goal location based on the path. Create global variables for storing the current pose and an index for tracking the iterations.
initPose_3= [path_3(1,1) path_3(1,2), 0];
...
for i=1:(dim_map_rows)
    for j=1:(dim_map_columns)
     ...
     random_matrix=0.01*rand(dim_map_rows,dim_map_columns);
...
%Creating a matrix where aij is the index of the closest center
A=zeros(dim_map_rows,dim_map_columns);

%Creating a matrix with the distance from the closest center for every
%point
D=zeros(dim_map_rows,dim_map_columns);

%In order to find the best dmax we need to knwow the average disrtance from
%all point in the space
Davg=zeros(dim_map_rows,dim_map_columns);
...
%notice that if the distance is less than dmax there will be one eta equals to 1 
...
dmax=30;
  ...
  for i=1:(dim_map_rows)
     for j=1:(dim_map_columns)
        ...
        %storing in a vector v the distance  from the centers
        v=[norm(p-c_1),norm(p-c_2),norm(p-c_3)];
        ...
        %creating a Matrix where in every i jth element lies the closest
        %center to the i j point on the map
        A(i,j)=a(1);
        
        %creating a matrix with the minimum distance from the closest
        %center (represent the d* in the algoorithm proposed)
        D(i,j)=min(v);
        
        %iterating for N robots (here N=3)
        for r=1:3
            if ( (v(r)>= D(i,j)) && (v(r)<= dmax) )
               Eta_matrix(i,j,r)=1+ 0.1*(D(i,j)-v(r))/(dmax-D(i,j));
            else
                Eta_matrix(i,j,r)=0;
            end
