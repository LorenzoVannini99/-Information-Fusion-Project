...
for i=1:(dim_map_rows)
    for j=1:(dim_map_columns)
     ...
     random_matrix=0.9*randn(dim_map_rows,dim_map_columns);
 ...
 for i=1:(dim_map_rows)
    for j=1:(dim_map_columns)
        ...
        if (matrix_map(i,j)<0.6)&&(matrix_map(i,j)>0.4)
...
fused_matrix=zeros(size(map_unc_2));
 ...
 for i=1:(dim_map_rows)
    for j=1:(dim_map_columns)
        ...
        den=( matrix_map_unc_3(i,j) * matrix_map_unc_2(i,j) * matrix_map_unc(i,j) )^(0.3333) + ( ( 1-matrix_map_unc_3(i,j) ) * ( 1-matrix_map_unc_2(i,j) ) * ( 1-matrix_map_unc(i,j) ) )^(0.3333);
